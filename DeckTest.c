...
float64_t average( Deck_float64_t* pile ) {
    float64_t sum = 0.0;
    float64_t num = pile->size;

    // for(LLNode* tmp=pile->Start; tmp!=NULL; tmp=tmp->next){
    for(LLNode_float64_t* tmp=pile->Start; tmp!=NULL; tmp=tmp->next){
        sum += tmp->data;
    }// linkedlist iteration
    return sum/num;
}

float64_t pop_average( Deck_float64_t* pile ) {
    // LLNode* temp = start;
    float64_t sum = 0.0;
    float64_t num = pile->size;
    float64_t* f64bufptr = zalloc(sizeof(float64_t));
    while(pile->size > 0){
        popDeckStart_float64_t(f64bufptr, pile);
        sum += *f64bufptr;
    }
    return sum/num;
}
...
static void test(int argc, char **argv){
    Deck_float64_t* pile = zalloc(sizeof(Deck_float64_t));
    float64_t* f64bufptr = zalloc(sizeof(float64_t));
    * f64bufptr = 9999.2;
    pushDeckStart_float64_t(f64bufptr, pile);
    popDeckStart_float64_t(f64bufptr, pile);
    print_f64(*f64bufptr);
